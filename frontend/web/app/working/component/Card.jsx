...
function Card({ index, title, description, img }) {
  ...
  return (
    <motion.div initial={"hidden"} whileInView="visible" transition={{ duration: 1, ease: 'easeOut' }}
      ...
      }} className={`flex flex-col lg:flex-row w-full lg:w-4/5 ${index % 2 === 0 ? "justify-start items-start text-start" : " justify-end items-end text-end"} ${index%2!==0?"bg-gradient-to-bl":"bg-gradient-to-tr"} from-[#F86F03] to-[#d79232] shadow border-gray-700 hover:bg-gray-700 p-5 rounded-md gap-5`} >
